 ...
 * in config.yml {@code animations}, and viewers who turned off {@link Setting#MATCH_PARTICLES} are skipped (they
 * don't get the effect's sounds either). The in-match titles, action bars and sounds are in {@link #fx()}.
 */
public final class Animations {
    ...
    /** Whether this viewer wants match particles ({@link Setting#MATCH_PARTICLES}). */
    private boolean particles(Player viewer) {
        PlayerProfile profile = plugin.profiles().get(viewer);
        return profile == null || profile.setting(Setting.MATCH_PARTICLES);
    }
    ...
    public void death(Location at, List<Player> viewers) {
        ...
        for (Player v : viewers) {
            if (!particles(v) || v.getWorld() != at.getWorld()) continue;
    ...
    public void roundWin(Player winner, List<Player> viewers) {
        ...
        for (Player v : viewers) {
            if (!particles(v) || v.getWorld() != base.getWorld()) continue;
            for (int i = 0; i < 24; i++) {
    ...
    public void matchWin(Player winner, List<Player> viewers) {
        ...
        for (int burst = 0; burst < 3; burst++) {
            ...
            plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
                ...
                for (Player v : viewers) {
                    if (!v.isOnline() || !particles(v) || v.getWorld() != c.getWorld()) continue;
