...
public final class Icons {
    ...
    /**
     * Parses "atlas:path" (e.g. "items:item/mace", "gui:hud/heart/full", "blocks:block/obsidian").
     * A bare path without atlas is treated as an item texture name.
     */
    public static ObjectComponent parse(String spec) {
        String s = spec.trim().toLowerCase(Locale.ROOT);
        int colon = s.indexOf(':');
        if (colon < 0) return item(s);
        return sprite(s.substring(0, colon), s.substring(colon + 1), "•");
    }
