...
// checks: the high-Elo match is listed first, the live (refreshed) list has no text box and its Search button opens the
// list with the search box, a name search narrows to one match, a search for nothing shows the no-results text, there
// is no sort input, and a click on a result starts spectating. Exit code 0 = pass.
...
async function main () {
  ...
  if (btns[0].id !== 'duelcore:spectate/find') throw new Error('search button is not first: ' + labels[0])
  if ((d.inputs || []).length) throw new Error('the live list must have no inputs (it is refreshed)')
  ...
  let m = L.mark(spec)
  L.click(spec, btns[0])
  const ds = await L.waitDialog(spec, /Live/, 5000, m.dialogs)
  const keys = (ds.inputs || []).map(i => i.key)
  if (!keys.includes('search') || keys.includes('sort')) throw new Error('expected only a search input: ' + keys)
  const search = L.buttons(ds)[0]
  if (search.id !== 'duelcore:spectate/search') throw new Error('search dialog: Search button is not first: ' + search.label)
  L.log('test', 'search-dialog-ok')

  // search by name → only the shield match
  await L.sleep(400)
  m = L.mark(spec)
  L.click(spec, search, { search: names[2] })
  ...
  m = L.mark(spec)
  L.click(spec, search, { search: 'shield' })
  ...
  await L.sleep(400) // server ignores clicks within 150 ms of the previous one
  m = L.mark(spec)
  L.click(spec, search, { search: 'zzzz_nobody' })
