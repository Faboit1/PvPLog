// Spectate menu: search + default Elo sort. Two ranked matches run at the same time (a high-rated sword pair
...
// no-results text, the sort input is present, and a click on a result starts spectating. Exit code 0 = pass.
...
async function main () {
  ...
  if (!keys.includes('search') || !keys.includes('sort')) throw new Error('missing inputs ' + keys)
  ...
  await L.sleep(400)
  let m = L.mark(spec)
  L.click(spec, btns[0], { search: names[2], sort: 'elo' })
  ...
  m = L.mark(spec)
  L.click(spec, btns[0], { search: 'shield', sort: 'newest' })
  ...
  await L.sleep(400) // server ignores clicks within 150 ms of the previous one
  m = L.mark(spec)
  L.click(spec, btns[0], { search: 'zzzz_nobody', sort: 'watchers' })
