...
- Hotbar items, each with an optional `action-bar` hint shown when it comes into the hand (the queue's
  "searching" bar has priority while queued; switching to an empty slot clears the hint).
